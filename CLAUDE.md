...
**Grounding is structural, not prompted**: every turn resolves against `source_span_ids`, and grading returns `Verdict.SUFFICIENT | INCOMPLETE | INCORRECT` by comparing the learner's explanation to those spans — never the model's general knowledge. `SUFFICIENT` ends the session; otherwise exactly one follow-up at the specific gap, never a spoon-fed correction, and never even when the learner asks for the answer outright. After `MAX_FOLLOWUPS` (3) the session closes by pointing at spans to review — never by revealing the answer. Two reinforcements: `domain/verbatim.py` catches near-verbatim recitation deterministically (string match, not an LLM call) and downgrades it to `INCOMPLETE`; and `prompts/schemas.py` orders `verdict` last so structured output must enumerate evidence before committing to a verdict.

**Identifiers are English, prose is Vietnamese**: enum members, enum values, JSON keys, WS message types and span codes are English/stable (`TurnState.STUDENT_RESPONDING`, `"sufficient"`, `[T06-138]`); everything the model reads as instruction or speaks to the learner is Vietnamese. Don't mix the two.
